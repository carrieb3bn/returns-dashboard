...
//   SHOPIFY_ADMIN_TOKEN (shpat_…), or SHOPIFY_CLIENT_ID + SHOPIFY_CLIENT_SECRET (client-credentials app).
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!token) return res.status(500).json({ code: 'not_configured', error: 'Set SHOPIFY_STORE and SHOPIFY_ADMIN_TOKEN (or client id/secret).' });
